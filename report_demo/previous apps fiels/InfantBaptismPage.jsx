...
import { generatePuppeteerInfantBaptismCertificate, formatDate, formatDateForInput } from '../utils/infantBaptismReportPuppeteer';
...
const InfantBaptismPage = ({
  ...
  const loadCertificates = async () => {
    setLoading(true);
    try {
      const result = await window.electron.infantBaptism.getCertificates({
  ...
  const loadNextCertificateNumber = async () => {
    try {
      const result = await window.electron.infantBaptism.getNextCertificateNumber({
        churchId: currentChurch.id,
        userId: user.id
      });
  ...
  const handleSaveCertificate = async () => {
    setSubmitting(true);
    try {
      const result = await window.electron.infantBaptism.createCertificate({
        certificateData: {
          ...formData,
  ...
  const handleGeneratePDF = async () => {
    ...
    try {
      ...
      const saveResult = await window.electron.infantBaptism.createCertificate({
        certificateData: {
          ...formData,
          church_id: currentChurch.id
        },
      ...
      // Then generate PDF
      const pdfResult = await generatePuppeteerInfantBaptismCertificate(
  ...
  const handlePrintCertificate = async (certificate) => {
    try {
      const result = await generatePuppeteerInfantBaptismCertificate(
        certificate,
        currentChurch,
        'print'
      );
  ...
  const handleDeleteCertificate = async (certificateId) => {
    try {
      const result = await window.electron.infantBaptism.deleteCertificate({
        certificateId,
        userId: user.id
      });
  ...
  return (
    <div className={styles.container}>
      ...
      <Breadcrumb
        pageTitle="Infant Baptism Certificate"
        breadcrumbs={[
          {
                ...
                <tbody>
                  {certificates.map((cert) => (
                    <tr key={cert.id}>
                      ...
                      <td className={styles.td}>{formatDate(cert.date_of_birth)}</td>
      ...
      <StatusBar
        ...
        currentView="infant-baptism"
      />
    </div>
  );
};

export default InfantBaptismPage;