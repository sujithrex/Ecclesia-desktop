class InfantBaptismService {
    ...
    async createCertificate(certificateData, userId) {
        try {
            ...
            const {
                ...
                sex, abode, father_name, mother_name, witness_name_1, witness_name_2,
            ...
            const result = await this.db.createInfantBaptismCertificate({
                ...certificateData,
                sex: sex.toLowerCase(),
                created_by: userId
            });
            ...
            console.error('Create infant certificate error:', error);
    ...
    async getCertificatesByChurch(churchId, userId, page = 1, limit = 8) {
        try {
            ...
            const certificates = await this.db.getInfantBaptismCertificatesByChurch(churchId, limit, offset);
            const totalCount = await this.db.getInfantBaptismCertificatesCount(churchId);
            ...
            console.error('Get infant certificates error:', error);
    ...
    async getCertificateById(certificateId, userId) {
        try {
            const certificate = await this.db.getInfantBaptismCertificateById(certificateId);
            ...
            console.error('Get infant certificate error:', error);
    ...
    async updateCertificate(certificateId, certificateData, userId) {
        try {
            ...
            const existing = await this.db.getInfantBaptismCertificateById(certificateId);
            ...
            const result = await this.db.updateInfantBaptismCertificate(certificateId, certificateData);
            ...
            console.error('Update infant certificate error:', error);
            if (error.message && error.message.includes('UNIQUE constraint failed')) {
                return { success: false, error: 'A certificate with this number already exists' };
            }
    ...
    async deleteCertificate(certificateId, userId) {
        try {
            ...
            const result = await this.db.deleteInfantBaptismCertificate(certificateId);
            ...
            console.error('Delete infant certificate error:', error);
    ...
    async getNextCertificateNumber(churchId, userId) {
        try {
            ...
            const nextNumber = await this.db.getNextInfantCertificateNumber(churchId);
            ...
            console.error('Get next infant certificate number error:', error);
    ...
    async getCertificateDataForPDF(certificateId, userId) {
        try {
            ...
            console.error('Get infant certificate data for PDF error:', error);
            return { success: false, error: 'Failed to get certificate data' };
        }
    }
}

module.exports = InfantBaptismService;