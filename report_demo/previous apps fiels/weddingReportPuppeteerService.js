...
class WeddingReportPuppeteerService {
    constructor(db) {
        this.db = db;
        this.templatePath = path.join(__dirname, 'templates', 'weddinglist.ejs');
        this.fontPath = path.join(__dirname, 'assets', 'fonts', 'Vijaya.ttf');
    }

    /**
     * Generate wedding anniversary PDF using Puppeteer
     ...
     */
    async generateWeddingPDF(reportData, church, dateRange, options = {}) {
        try {
...
module.exports = WeddingReportPuppeteerService;
